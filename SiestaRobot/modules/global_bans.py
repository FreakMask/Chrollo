...
def gban(update: Update, context: CallbackContext):
    ...
    if sql.is_user_gbanned(user_id):
        ...
        if not reason:
            message.reply_text(
                "This user is already Murdered; I'd change the reason, but you haven't given me one...",
            )
            return
        ...
        if old_reason:
            message.reply_text(
                "This user is already Murdered and Gbanned for the following reason:\n"
                "<code>{}</code>\n"
            ...
            message.reply_text(
                "This user is already murdered, but had no reason set; I've gone and updated it!",
            )
    ...
    log_message = (
        ...
        f"<b>Spider:</b> {mention_html(user.id, user.first_name)}\n"
    ...
    for chat in chats:
        ...
        try:
            ...
            if excp.message in GBAN_ERRORS:
                ...
                message.reply_text(f"Could not murder due to: {excp.message}")
                if EVENT_LOGS:
                    bot.send_message(
                        EVENT_LOGS,
    ...
    try:
        bot.send_message(
            user_id,
            "#EVENT"
            "You have been marked as an Enemy and as such have been banned from any future groups we manage."
...
def ungban(update: Update, context: CallbackContext):
    ...
    for chat in chats:
        ...
        try:
            ...
            if excp.message in UNGBAN_ERRORS:
                ...
                message.reply_text(f"Could not recall due to: {excp.message}")
                if EVENT_LOGS:
                    bot.send_message(
                        EVENT_LOGS,
                        f"Could not recall due to: {excp.message}",
                        parse_mode=ParseMode.HTML,
                    )
                else:
                    bot.send_message(
    ...
    if EVENT_LOGS:
        ...
        send_to_list(bot, DRAGONS + DEMONS, "recall complete!")
    ...
    if ungban_time > 60:
        ...
        message.reply_text(f"Person has been Reincarnated. Took {ungban_time} min")
    else:
        message.reply_text(f"Person has been Reincarnated. Took {ungban_time} sec")
...
def gbanlist(update: Update, context: CallbackContext):
    ...
    if not banned_users:
        update.effective_message.reply_text(
            "There aren't any Murdered users! You're kinder than I expected...",
        )
        return
    ...
    with BytesIO(str.encode(banfile)) as output:
        output.name = "Murder List.txt"
        update.effective_message.reply_document(
            document=output,
            filename="Murder List.txt",
...
def check_and_ban(update, user_id, should_message=True):
    ...
    if sw_ban:
        ...
        if should_message:
            update.effective_message.reply_text(
                f"<b>Alert</b>: this user have been Murdered and Gbanned.\n"
